...
import org.apache.flink.table.types.{DataType, logical}
...
object AvroSerDe {
  ...
  def getRowSchema(topic: String): TypeInformation[Row] = {
  ...
  def getRowDataType(topic: String): DataType = {
  ...
  def convertToTypeInfo(fieldSchema: Schema): TypeInformation[_] = {
    fieldSchema.getType match {
      ...
      case UNION => {
        if (
          fieldSchema.getTypes.size == 2 && (fieldSchema.getTypes
            .get(0)
            .getType == Schema.Type.NULL)
        ) convertToTypeInfo(fieldSchema.getTypes.get(1))
        else if (
          fieldSchema.getTypes.size == 2 && (fieldSchema.getTypes
            .get(1)
            .getType == Schema.Type.NULL)
        ) convertToTypeInfo(fieldSchema.getTypes.get(0))
      ...
      case UNION =>
        ...
        if (size == 2 && (types.get(0).getType == Schema.Type.NULL))
          ...
          schema.getTypes.asScala
            ...
            .zipWithIndex
            .foreach(x =>
              row.setField("member" + x._2, convertAvroType(x._1, fieldInfo(x._2), `object`))
            )
    ...
    val fieldDataType: DataType = schema.getType match {
      ...
      case UNION =>
        if (schema.getTypes.size == 2 && (schema.getTypes.get(0).getType == Schema.Type.NULL)) {
          ...
          schema.getTypes.size == 2 && (schema.getTypes.get(1).getType == Schema.Type.NULL)
        ) {
...
//    TODO handle rowName clash in different fields
    ...
    logicalType.getTypeRoot match {
      ...
      case logical.LogicalTypeRoot.INTEGER | logical.LogicalTypeRoot.TINYINT |
...
//          TODO don't use avro logical type
      ...
      case logical.LogicalTypeRoot.ROW =>
        ...
        for (i <- 0 until rowType.getFieldCount) {
          ...
          if (!fieldName.equalsIgnoreCase("rowtime") && !fieldName.equalsIgnoreCase("proctime")) {
  ...
  private def nullableSchema(schema: Schema) = if (schema.isNullable) schema
